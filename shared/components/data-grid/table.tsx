...
export const Table = ({
  ...
  const hanlechangeSelect = async (e: any, value: any, columnName: any, rowId: any) => {
    let id = rowId;
    let values = {
      id: rowId,
      column: columnName,
          ...
          <tr>
            ...
            {Children.map(children, (child, key) => {
              ...
              if (child.type.name === Column.name) {
                const { title, className, sortKey, style, typeColumn, filterName, filterOption, isShowFilter } = child.props;
                    ...
                    >
                      {!noSelected && (
                        <td className="w-4 p-3 border border-slate-300">
                          {
                            !item.hideCheck && <div className="flex items-center grid-selected">
                              <input
                      ...
                      {Children.map(children, (child, key) => {
                        ...
                        if (child.type.name === Column.name) {
                          ...
                          return (
                            <td style={{ cursor: "pointer" }} contentEditable={editLine}
                              onBlur={(e) => { onBlurEditData(e, body({ item, index })) }}
                              columnTitle={item.id} filterName={filterName}
                              key={key}
                              className={[
                              ...
                              {(() => {
                                if (typeColumn == 'text') {
                                  ...
                                  if (editSelect === item.id) {
                                    return (
                                      <Select2
                                        ...
                                        onChange={(e: any) => {
                                          hanlechangeSelect(e, body({ item, index }), filterName, item.id);
                                        }}
                                      />
                                    )
                                  } else {
                                    return (
                                      <span onClick={(e) => { onClickEditSelect(e, item.id) }}>{body({ item, index })} </span>
                                    )
                                  }
                                  ...
                                  if (editBoolean === item.id) {
                                    return (
                                      <>
                                        <input type="checkbox" defaultChecked={item.isShow ? true : false} name={filterName}
                                          onChange={(e) => {
                                            hanlechangeBoolean(e, body({ item, index }));
                                          }}
                                        /> Hiển thị
                                      </>
                                    )
                                    ...
                                    return (
                                      <span onClick={(e) => { onClickEditBoolean(e, item.id) }}>{body({ item, index })} </span>
                                    )
                                  }
                                } else {
                                  return (
