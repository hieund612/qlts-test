...
import { donViChoThueServices } from "../_services/services";
import { formReducer, INITIAL_STATE_FORM, computedTitle } from "@/lib/common";
export default function DonViChoThueForm({
    show,
    action,
    id,
    onClose,
    ...
    const { data, error, isLoading, mutate } = donViChoThueServices.GetById(id!);
    const [loading, setLoading] = useState(false);
    const onSubmit = async (values: any) => {
        setLoading(true);
        if (id) {
            try {
                await donViChoThueServices.updatewithfile(values, 'DonViChoThueUpdateDto', [{ name: 'fileDinhKems', file: 'fileDinhKemAttachs' }]);
            ...
            try {
                await donViChoThueServices.createwithfile(values, 'DonViChoThueCreateDto', [{ name: 'fileDinhKems', file: 'fileDinhKemAttachs' }]);
