...
import { hienTrangServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import HienTrangForm from "./_components/hientrang-form";
...
export default function Page() {
    ...
    const titleTable = "Hiện trạng";
    const { data, isLoading, mutate } = hienTrangServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("hientrang"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã hiện trạng" sortKey="maHT" body={({ item }) => (<span>{item.maHT}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên hiện trạng" sortKey="tenHT" body={({ item }) => (<span>{item.tenHT}</span>)} />
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, hienTrangServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <HienTrangForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
