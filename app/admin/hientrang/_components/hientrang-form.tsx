...
export default function HienTrangForm({
    show,
    action,
    id,
    onClose,
    ...
    const schema = object({
        maHT: string().trim().nullable().required('Mã hiện trạng không được để trống').max(20, 'Bạn nhập tối đa 20 ký tự'),
        tenHT: string().trim().nullable().required('Tên hiện trạng không được để trống').max(250, 'Bạn nhập tối đa 250 ký tự'),
        moTa: string().trim().nullable().max(1000, 'Bạn nhập tối đa 1000 ký tự'),
    });
    const [state, dispatch] = useReducer(formReducer, INITIAL_STATE_FORM);
    const { data, error, isLoading, mutate } = hienTrangServices.GetById(id!);
    const [loading, setLoading] = useState(false);
    const onSubmit = async (values: any) => {
        setLoading(true);
        if (id) {
            try {
                await hienTrangServices.update(id, values);
            ...
            try {
                await hienTrangServices.create(values);
