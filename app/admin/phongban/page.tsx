...
import { phongBanServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import PhongBanForm from "./_components/phongban-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = phongBanServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("phongban"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã phòng ban" sortKey="maPB" body={({ item }) => (<span>{item.maPB}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên phòng ban" sortKey="tenPB" body={({ item }) => (<span>{item.tenPB}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, phongBanServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <PhongBanForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
