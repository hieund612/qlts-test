...
import DinhDangForm from "./_components/dinhdang-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = dinhDangServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("dinhdang"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    <GridView.Table.Column style={{}} title="STT" sortKey="stt" body={({ item }) => (<span>{item.stt}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, dinhDangServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <DinhDangForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
