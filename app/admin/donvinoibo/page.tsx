...
import DonViNoiBoForm from "./_components/donvinoibo-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = donViNoiBoServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("donvinoibo"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã đơn vị" sortKey="maDV" body={({ item }) => (<span>{item.maDV}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên đơn vị" sortKey="tenDV" body={({ item }) => (<span>{item.tenDV}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, donViNoiBoServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <DonViNoiBoForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
