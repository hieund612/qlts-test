...
import { trangThaiSuDungServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import TrangThaiSuDungForm from "./_components/trangthaisudung-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = trangThaiSuDungServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("trangthaisudung"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã trạng thái" sortKey="maTT" body={({ item }) => (<span>{item.maTT}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên trạng thái" sortKey="tenTT" body={({ item }) => (<span>{item.tenTT}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, trangThaiSuDungServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <TrangThaiSuDungForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
