  ...
  return (
    <>
      <Modal show={show} size="xxl" loading={isLoading}>
        <>
          <Modal.Header onClose={onClose}>Cập nhật người dùng cho nhóm: {dataGroup?.title}</Modal.Header>
                ...
                <GridView.Table.Column
                  ...
                  body={({ item }) => (
                    <div className="flex flex-row">
                      {permisson.per_Edit && !item.lstGroupIds.includes(id) && <FaUserPlus
                        className="cursor-pointer text-lg mr-1 text-blue-800"
                        title="Thêm"
                        onClick={() => onAddUser(item.id)}
                      />}
                      {permisson.per_Edit && item.lstGroupIds.includes(id) && <FaUserTimes
