...
import { nguonGocServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import NguonGocForm from "./_components/nguongoc-form";
...
export default function Page() {
    ...
    const titleTable = "Nguồn gốc hình thành";
    const { data, isLoading, mutate } = nguonGocServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("nguongoc"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã nguồn gốc" sortKey="maNG" body={({ item }) => (<span>{item.maNG}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên nguồn gốc" sortKey="tenNG" body={({ item }) => (<span>{item.tenNG}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, nguonGocServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <NguonGocForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
