...
import { lyDoBienDongServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import LyDoBienDongForm from "./_components/lydobiendong-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = lyDoBienDongServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("lydobiendong"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã lý do biến động" sortKey="maLyDoBD" body={({ item }) => (<span>{item.maLyDoBD}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên lý do biến động" sortKey="tenLyDoBD" body={({ item }) => (<span>{item.tenLyDoBD}</span>)} />
                    <GridView.Table.Column style={{}} title="Loại biến động" sortKey="loaiBD_Id" body={({ item }) => (<span>{item.loaiBD}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, lyDoBienDongServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <LyDoBienDongForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
