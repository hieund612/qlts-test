...
import { tinhHinhKhaiThacServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import TinhHinhKhaiThacForm from "./_components/tinhhinhkhaithac-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = tinhHinhKhaiThacServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("tinhhinhkhaithac"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã tình hình khai thác" sortKey="maTH" body={({ item }) => (<span>{item.maTH}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên tình hình khai thác" sortKey="tenTH" body={({ item }) => (<span>{item.tenTH}</span>)} />
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, tinhHinhKhaiThacServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <TinhHinhKhaiThacForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
