...
export default function Page() {
  ...
  const { data, isLoading, mutate } = taiSanSapXepServices.GetList(meta);
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("taisansapxep"));
  }, []);
  return (
    <>
	...
	/>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}}title="Phương án đề xuất"sortKey="phuongAnDeXuat"body={({ item }) => (<span>{item.phuongAnDeXuat}</span>)} />
<GridView.Table.Column style={{}}title="Trạng thái"sortKey="trangThai"body={({ item }) => (<span>{item.trangThai}</span>)} />
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,taiSanSapXepServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      <TaiSanSapXepForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
