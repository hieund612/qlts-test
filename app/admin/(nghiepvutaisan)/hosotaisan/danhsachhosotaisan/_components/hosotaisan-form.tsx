...
export default function HoSoTaiSanForm({
  ...
  const onSubmit = async (values: any) => {
    ...
    setLoading(true);
    try {
      await hoSoTaiSanServices.thayDoiThongTin(values, 'HiHoSoTaiSanUpdateDto', [{ name: 'fileDinhKems', file: 'fileDinhKemAttachs' }]);
      toast.success("Thay đổi thông tin GCN thành công");
      await mutate();
      await onClose(true);
      window.location.href="/admin/hosotaisan/thaydoithongtin"
    } catch (err: any) {
        ...
        >
          {({ handleSubmit, values, setFieldValue }) => (
            <Form noValidate
              ...
              <Modal.Header onClose={onClose}>Chi tiết hồ sơ tài sản</Modal.Header>
              <Modal.Body nameClass="grid-cols-12">
                ...
                <div className='col-span-2'>
                  <TanetLabel
                    label={(action=="read"||data?.phienBan==1)?'Ngày phát hành':'Ngày thay đổi'}
                    required={true}
                  /></div>
                <div className='col-span-4'>
                  <TanetFormDate
                    ...
                    id={(action=="read"||data?.phienBan==1)?'ngayPhatHanh':'ngayThayDoi'}
                    name={(action=="read"||data?.phienBan==1)?'ngayPhatHanh':'ngayThayDoi'}
                  /></div>
                <div className='col-span-2'>
                  <TanetLabel
