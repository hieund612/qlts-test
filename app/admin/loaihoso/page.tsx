...
import LoaiHoSoForm from "./_components/loaihoso-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = loaiHoSoServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("loaihoso"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã loại hồ sơ" sortKey="maLoaiHS" body={({ item }) => (<span>{item.maLoaiHS}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên loại hồ sơ" sortKey="tenLoaiHS" body={({ item }) => (<span>{item.tenLoaiHS}</span>)} />
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, loaiHoSoServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <LoaiHoSoForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
