...
import { phuongAnSXServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import PhuongAnSXForm from "./_components/phuongansx-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = phuongAnSXServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("phuongansx"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã phương án sản xuất" sortKey="maPASX" body={({ item }) => (<span>{item.maPASX}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên phương án sản xuất" sortKey="tenPASX" body={({ item }) => (<span>{item.tenPASX}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, phuongAnSXServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <PhuongAnSXForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
