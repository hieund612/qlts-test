...
export default function ThoiDiemCPHForm({
    show,
    action,
    id,
    onClose,
    ...
    const schema = object({
        maThoiDiemCPH: string().trim().nullable().required('Mã thời điểm cổ phần hóa không được để trống').max(20, 'Bạn nhập tối đa 20 ký tự'),
        tenThoiDiemCPH: string().trim().nullable().required('Tên thời điểm cổ phần hóa không được để trống').max(250, 'Bạn nhập tối đa 250 ký tự'),
    });
    const [state, dispatch] = useReducer(formReducer, INITIAL_STATE_FORM);
    const { data, error, isLoading, mutate } = thoiDiemCPHServices.GetById(id!);
    const [loading, setLoading] = useState(false);
    const onSubmit = async (values: any) => {
        setLoading(true);
        if (id) {
            try {
                await thoiDiemCPHServices.update(id, values);
            ...
            try {
                await thoiDiemCPHServices.create(values);
                        ...
                        <Form noValidate
                            ...
                            <Modal.Body nameClass="grid-cols-12">
                                <div className='col-span-12'>
                                    <TanetInput
                                        label='Mã thời điểm cổ phần hóa'
                                        required={true}
                                        view={state?.viewMode}
                                        id='maThoiDiemCPH'
                                        name='maThoiDiemCPH'
                                ...
                                <div className='col-span-12'>
                                    <TanetInput
                                        label='Tên thời điểm cổ phần hóa'
                                        required={true}
                                        view={state?.viewMode}
                                        id='tenThoiDiemCPH'
                                        name='tenThoiDiemCPH'
