...
import { diaDiemServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import DiaDiemForm from "./_components/diadiem-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = diaDiemServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("diadiem"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã địa bàn" sortKey="maDiaBan" body={({ item }) => (<span>{item.maDiaBan}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên địa bàn" sortKey="tenDiaBan" body={({ item }) => (<span>{item.tenDiaBan}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, diaDiemServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <DiaDiemForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
