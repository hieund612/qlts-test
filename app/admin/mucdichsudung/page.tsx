...
import { mucDichSuDungServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import MucDichSuDungForm from "./_components/mucdichsudung-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = mucDichSuDungServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("mucdichsudung"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã mục đích" sortKey="maMD" body={({ item }) => (<span>{item.maMD}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên mục đích" sortKey="tenMD" body={({ item }) => (<span>{item.tenMD}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, mucDichSuDungServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <MucDichSuDungForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
