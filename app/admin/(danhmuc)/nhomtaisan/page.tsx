...
import { nhomTaiSanServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import NhomTaiSanForm from "./_components/nhomtaisan-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = nhomTaiSanServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("nhomtaisan"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã nhóm tài sản" sortKey="maNhomTS" body={({ item }) => (<span>{item.maNhomTS}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên nhóm tài sản" sortKey="tenNhomTS" body={({ item }) => (<span>{item.tenNhomTS}</span>)} />
                    <GridView.Table.Column style={{}} title="Tỷ lệ khấu hao (%)" sortKey="tyLeKhauHao" body={({ item }) => (<span>{item.tyLeKhauHao}</span>)} />
                    <GridView.Table.Column style={{}} title="Số năm sử dụng" sortKey="thoiGianSD" body={({ item }) => (<span>{item.thoiGianSD}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, nhomTaiSanServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <NhomTaiSanForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
