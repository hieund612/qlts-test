...
import LoaiTaiSanForm from "./_components/loaitaisan-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = loaiTaiSanServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("loaitaisan"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã loại tài sản" sortKey="maLoaiTS" body={({ item }) => (<span>{item.maLoaiTS}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên loại tài sản" sortKey="tenLoaiTS" body={({ item }) => (<span>{item.tenLoaiTS}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, loaiTaiSanServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <LoaiTaiSanForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
