...
import { tinhTrangPASXServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import TinhTrangPASXForm from "./_components/tinhtrangpasx-form";
...
export default function Page() {
    ...
    const titleTable = "Tình trạng phê duyệt phương án sản xuẩt";
    const { data, isLoading, mutate } = tinhTrangPASXServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("tinhtrangpasx"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã tình trạng" sortKey="maTT" body={({ item }) => (<span>{item.maTT}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên tình trạng" sortKey="tenTT" body={({ item }) => (<span>{item.tenTT}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, tinhTrangPASXServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <TinhTrangPASXForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
