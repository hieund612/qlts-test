...
export default function NguonGocForm({
    show,
    action,
    id,
    onClose,
    ...
    const schema = object({
        maNG: string().trim().nullable().required('Mã nguồn gốc không được để trống').max(20, 'Bạn nhập tối đa 20 ký tự'),
        tenNG: string().trim().nullable().required('Tên nguồn gốc không được để trống').max(250, 'Bạn nhập tối đa 250 ký tự'),
        moTa: string().trim().nullable().max(1000, 'Bạn nhập tối đa 1000 ký tự'),
    });
    const [state, dispatch] = useReducer(formReducer, INITIAL_STATE_FORM);
    const { data, error, isLoading, mutate } = nguonGocServices.GetById(id!);
    const [loading, setLoading] = useState(false);
    const onSubmit = async (values: any) => {
        setLoading(true);
        if (id) {
            try {
                await nguonGocServices.update(id, values);
            ...
            try {
                await nguonGocServices.create(values);
