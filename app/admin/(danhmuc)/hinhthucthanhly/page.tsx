...
import HinhThucThanhLyForm from "./_components/hinhthucthanhly-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = hinhThucThanhLyServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("hinhthucthanhly"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã hình thức" sortKey="maHT" body={({ item }) => (<span>{item.maHT}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên hình thức" sortKey="tenHT" body={({ item }) => (<span>{item.tenHT}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, hinhThucThanhLyServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <HinhThucThanhLyForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
