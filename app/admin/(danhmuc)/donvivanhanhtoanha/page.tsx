...
import { donViVanHanhToaNhaServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import DonViVanHanhToaNhaForm from "./_components/donvivanhanhtoanha-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = donViVanHanhToaNhaServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("donvivanhanhtoanha"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Tên đơn vị vận hành tòa nhà" sortKey="tenDV" body={({ item }) => (<span>{item.tenDV}</span>)} />
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, donViVanHanhToaNhaServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <DonViVanHanhToaNhaForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
