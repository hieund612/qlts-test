...
import ThoiHanSuDungForm from "./_components/thoihansudung-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = thoiHanSuDungServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("thoihansudung"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã thời hạn" sortKey="maTH" body={({ item }) => (<span>{item.maTH}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên thời hạn" sortKey="tenTH" body={({ item }) => (<span>{item.tenTH}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, thoiHanSuDungServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <ThoiHanSuDungForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
