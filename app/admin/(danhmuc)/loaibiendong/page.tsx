...
import { loaiBienDongServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import LoaiBienDongForm from "./_components/loaibiendong-form";
...
export default function Page() {
    ...
    const titleTable = "Loại biến động";
    const { data, isLoading, mutate } = loaiBienDongServices.GetList(meta);
    ...
    useEffect(() => {
        setPermisson(getPermisson("loaibiendong"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã loại biến động" sortKey="maLoaiBD" body={({ item }) => (<span>{item.maLoaiBD}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên loại biến động" sortKey="tenLoaiBD" body={({ item }) => (<span>{item.tenLoaiBD}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, loaiBienDongServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <LoaiBienDongForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
