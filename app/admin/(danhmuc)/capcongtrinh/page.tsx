...
import { capCongTrinhServices } from "./_services/services";
import {
    AiOutlinePlus,
    AiFillEdit,
    AiFillDelete,
...
import CapCongTrinhForm from "./_components/capcongtrinh-form";
...
export default function Page() {
    ...
    const { data, isLoading, mutate } = capCongTrinhServices.GetList(meta);
    ...
    console.log('log:', 1);
    useEffect(() => {
        setPermisson(getPermisson("capcongtrinh"));
    }, []);
    return (
        <>
            <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
                ...
                >
                    ...
                    />
                    <GridView.Table.Column style={{}} title="Mã cấp" sortKey="maCap" body={({ item }) => (<span>{item.maCap}</span>)} />
                    <GridView.Table.Column style={{}} title="Tên cấp" sortKey="tenCap" body={({ item }) => (<span>{item.tenCap}</span>)} />
                    <GridView.Table.Column
                        style={{ width: "10%" }}
                        ...
                        body={({ item }) => (
                            <div className="flex flex-row">
                                ...
                                {permisson.per_Delete && <AiFillDelete
                                    ...
                                    title="Xóa"
                                    onClick={() => delAction(item, capCongTrinhServices, data, setMeta, meta, mutate)}
                                />
                                }
                            </div>
                        )}
            ...
            <CapCongTrinhForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
            <ConfirmationDialog />
        </>
    );
}
